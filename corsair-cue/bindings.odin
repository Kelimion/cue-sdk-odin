...
when        ODIN_OS == "windows" {
	LIBRARY_NAME :: "CUESDK.x64_2017.dll"
} else when ODIN_OS == "darwin" {
	LIBRARY_NAME :: "libCUESDK.dylib"
} else {
	#panic("Corsair CUE SDK is not supported on Linux, so we have no bindings for them.")
}
...
init :: proc(file_path := string("")) -> (ok: bool) {
	...
	if sdk.handle, ok = dynlib.load_library(file_path); !ok { return false }
	...
	sdk.set_leds_colors_buffer_by_device_index = transmute(Set_Leds_Colors_Buffer_By_Device_Index)symbol

	if symbol, symbol_ok = dynlib.symbol_address(sdk.handle, "CorsairSetLedsColorsFlushBuffer");         !symbol_ok { return false }
	sdk.set_leds_colors_flush_buffer           = transmute(Set_Leds_Colors_Flush_Buffer)symbol
...
get_bool_property_value :: proc(device_index: i32, property_id: Device_Property_Id) -> (value: bool, ok: bool) {
	val: b32
	ok = bool(sdk.get_bool_property_value(device_index, property_id, &val))
...
get_i32_property_value :: proc(device_index: i32, property_id: Device_Property_Id) -> (value: i32, ok: bool) {
	val: i32
	ok = bool(sdk.get_i32_property_value(device_index, property_id, &val))
...
set_leds_colors_async :: proc(led_colors: []Led_Color, callback: Async_Callback, ctx: rawptr) {
	sdk.set_leds_colors_async(i32(len(led_colors)), raw_data(led_colors), callback, ctx)
}
...
set_leds_colors_buffer_by_device_index :: proc(device_index: i32, led_colors: []Led_Color) {
	sdk.set_leds_colors_buffer_by_device_index(device_index, i32(len(led_colors)), raw_data(led_colors))
}
...
get_leds_colors_by_device_index :: proc(device_index: i32, led_colors: []Led_Color) {
	sdk.get_leds_colors_by_device_index(device_index, i32(len(led_colors)), raw_data(led_colors))
}
...
get_led_positions_by_device_index :: proc(device_index: i32) -> (led_positions: []Led_Position) {
	positions := sdk.get_led_positions_by_device_index(device_index)
...
get_led_positions :: proc() -> (led_positions: []Led_Position) {
	positions := sdk.get_led_positions()
	return positions.led_position[:positions.number_of_leds]
}
